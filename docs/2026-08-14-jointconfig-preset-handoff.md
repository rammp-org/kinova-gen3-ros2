...
1. **Spec:** `docs/superpowers/specs/2026-08-14-goto-jointconfig-preset-design.md` (design + rationale).
1. Prior tier for reference: `docs/superpowers/specs/2026-08-12-...` and the shipped `GoToEEPose` (spec `2026-08-14-goto-ee-pose-curobo-design.md`, plan `2026-08-14-goto-ee-pose.md`).
...
- `GoToEEPose` tier is **PR #2** (`rammp-org/kinova_gen3_ros2`), sim-validated, real-arm pending. Its base now includes the RT-core fix `79d1050` (`--cpu 11` pinning + build scoping).
...
1. **Both new actions plan through cuRobo `plan_to_joints`** (collision-aware) — NOT a planner-bypass direct move. `CuroboPlanClient` gains `plan_to_joints` (+ type-erased `cancel()`).
1. **GoToPreset = a named joint config.** Server resolves `preset_name → 7 joints` from a registry (ROS params `preset_names` + `presets.<name>`, default `home` = cuRobo retract), then plans via `plan_to_joints`.
1. New `.action` Result/Feedback are byte-identical to `GoToEEPose` (so the templated base sets them uniformly). Reuse `kPlanningFailed`.
...
- **Builds aarch64-only on abra.** Dev loop: `bash scripts/abra_colcon.sh --packages-up-to kinova_gen3_ros2 --cmake-args -DBUILD_TESTING=ON`; run a gtest binary via `ssh abra 'bash -lc "source /opt/ros/humble/setup.bash; source /tmp/kinova-ros2-ws/install/setup.bash; /tmp/kinova-ros2-ws/build/kinova_gen3_ros2/<TESTBIN>"'`.
...
- **Push over HTTPS via gh** (no SSH key this session): `GIT_CONFIG_GLOBAL=/dev/null git -c 'credential.helper=!gh auth git-credential' push https://github.com/rammp-org/kinova_gen3_ros2.git <branch>`; `gh pr create --repo … --base feat/goto-ee-pose-curobo --head feat/arm-goto-jointconfig-preset`.
...
`kinova-gen3-ros2-repo`, `arm-interface-layer`, `explain-tradeoffs-plainly` (pitch design forks in plain language, not jargon).

...
```sh
sudo apt install -y git python3-pip python3-vcstool ros-humble-rmw-cyclonedds-cpp
rosdep update --rosdistro humble        # first time only
...
`kinova_gen3.repos` brings in the core driver, the interfaces, and cuRobo's message
definitions. It does **not** contain this repo, which is why you clone that yourself
first.
...
export CMEEL_PREFIX="$(python3 -c 'import sysconfig; print(sysconfig.get_paths()["purelib"])')/cmeel.prefix"
```
...
colcon build --packages-up-to kinova_gen3_ros2 --cmake-args -DCMAKE_BUILD_TYPE=Release
```

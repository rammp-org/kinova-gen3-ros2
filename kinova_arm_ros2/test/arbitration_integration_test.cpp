...
struct RecordingSink : public CommandSink, public StreamSink {
  ...
  StreamOpenResult on_stream_open(const StreamOpenRequest&) override {
    note("stream_open");
    // Stands in for Supervisor's mode settle, which really does sleep 250 ms inside
    // this call while the Arbiter mutex is held.
...
// A read must work with no owner and while e-stopped, or it is useless for working out
...
// arbitration tier could only approximate with a blocked goal: a real on_stream_open
// holding the Arbiter mutex through its mode settle while an e-stop arrives.
//
// As in the goal version, this measures the HALT ARRIVING downstream, not how long

#!/usr/bin/env python3
"""Exercise every ROS control surface against a RUNNING kinova_arm_node.

    ros2 run kinova_arm_ros2 kinova_arm_node --sim --urdf ...     # or the real arm
...
def main() -> int:
    ...
    if ctx.control_status(fresh=False) is None:
        print(f"{RED}no /control_status. Is kinova_arm_node running?{RESET}")
        rclpy.shutdown()
        return 2

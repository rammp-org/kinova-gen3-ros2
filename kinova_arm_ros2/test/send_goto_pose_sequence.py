...
def sample_poses(n, seed):
    ...
    return [{"name": f"rand_{i + 1}",
             "pos": [round(rng.uniform(*WS_X), 3),
...
_CODES = {0: "SUCCESSFUL", -1: "INVALID_GOAL", -4: "PATH_TOLERANCE_VIOLATED",
...
def _send_one(node, client, pose, sender_id):
    ...
    goal.target.pose.position.x, goal.target.pose.position.y, goal.target.pose.position.z = pose["pos"]
    (goal.target.pose.orientation.x, goal.target.pose.orientation.y,
     goal.target.pose.orientation.z, goal.target.pose.orientation.w) = _normalize(pose["quat"])
    ...
    node.get_logger().info(f"  result={label} ({res.error_code}) msg='{res.error_string}'")
...
def main():
    ...
    ap.add_argument("--poses", metavar="FILE",
    ...
    if not _HAVE_ROS:
        print("\n--go needs a ROS2 environment (rclpy + kinova_arm_interfaces); none found.")
        return 1
        ...
        for i, pose in enumerate(poses):
            ...
            if code != 0:
                ...
                if not args.continue_on_fail:
                    node.get_logger().error(f"stopping: {name} did not succeed (code {code})")
                    break
    finally:
        rclpy.shutdown()

...
// cartesian_impedance is unavailable, and core has no kind for it at all -- so the
// rejection has to originate here, not in pair_supported().
...
// so the surface is complete; this guards against someone later wiring wrench into the
// wrong on_setpoint_* method.
...
// open/timeout/rejected_count come from core, NOT from what this node remembers.

...
from kinova_arm_interfaces.msg import JointSetpoint, TwistSetpoint
from kinova_arm_interfaces.srv import AcquireControl, CloseStream, OpenStream, ReleaseControl
...
class Probe(Node):
    def __init__(self):
        ...
        self.create_subscription(JointState, "/joint_states",
                                 lambda m: setattr(self, "q", list(m.position)), SENSOR_QOS)
        self.pub_jv = self.create_publisher(JointSetpoint, "/setpoint/joint_velocity", SP_QOS)
    ...
    def run_case(self, controller):
        ...
        if not r.accepted:
            ...
            return None
        print(f"  open on {list(r.channels)}; |q0 - q_rest| = {norm(q0, Q_REST):.3f} rad")

        end = time.time() + SECONDS
        nxt = time.time()
        while time.time() < end:
            if controller == "joint_velocity":
                self.pub_jv.publish(JointSetpoint(values=[0.0] * 7,
                                                  token=[int(x) for x in self.token]))
            else:
                self.pub_tw.publish(TwistSetpoint(twist=Twist(),
                                                  token=[int(x) for x in self.token]))
            if time.time() >= nxt:
                print(f"    t={SECONDS - (end - time.time()):4.1f}s  "
        ...
        print(f"    |q-q_rest| {norm(q0, Q_REST):.3f} -> {norm(q1, Q_REST):.3f}"
...
def main():
    ...
    r = n.call(AcquireControl, "acquire_control",
               AcquireControl.Request(owner_id="zero_probe"))
    if not r.accepted:
    ...
    try:
        ...
        if jv is not None and tw is not None:
            if tw > 5 * max(jv, 1e-4):
                print(f"  joint_velocity held ({jv:.4f} rad), ee_twist drifted ({tw:.4f} rad)")
                print("  => the DLS solve / null-space posture term is driving the arm.")
                print("     The actuator velocity servo is NOT at fault.")
            elif jv > 0.05 and tw > 0.05:
                print(f"  BOTH drifted (jv={jv:.4f}, twist={tw:.4f} rad)")
                print("  => a zero velocity command does not hold: the servo is not tracking zero.")
            else:
                print(f"  both held (jv={jv:.4f}, twist={tw:.4f} rad) -- not reproduced here.")

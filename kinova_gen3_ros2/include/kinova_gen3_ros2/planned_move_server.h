...
#include "kinova_gen3_ros2/joint_point.h"
...
public:
  ...
  PlannedMoveServer(rclcpp::Node::SharedPtr node,
                    const std::string &action_name, GoalRouter &router,
  ...
  // --- ActionServerPort (execution phase, supervisor sampler thread via
  // router) ---
  void
  ...
  void settle(const kinova::interface::GoalId &id,
    ...
    std::shared_ptr<GoalHandle> gh;
    {
      ...
      auto it = goals_.find(id);
      if (it == goals_.end())
        return; // already settled -> never terminal twice
      gh = it->second.gh;
      goals_.erase(it);
    }
...
protected:
  ...
  std::vector<double> start_config() const {
    ...
    // measured state, so there is no empty-vector fallback here -- returning
    // one would silently restore the /joint_states coupling this replaced.
...
private:
  ...
  handle_goal(const rclcpp_action::GoalUUID &,
    ...
    // read, and a default-constructed ArmState is {q = Zero, stamp_s = 0}.
    // Sending that q as the plan's start state would be indistinguishable from
    // a real measurement, and cuRobo would plan from the fully-extended zero
    // pose.
  ...
  rclcpp_action::CancelResponse handle_cancel(std::shared_ptr<GoalHandle> gh) {
    ...
    kinova::interface::Token token{};
    {
      ...
      auto it = goals_.find(id);
      if (it != goals_.end()) {
        executing = it->second.executing;
        token = it->second.token;
        it->second.cancel_requested = true;
      }
    }
    if (executing) {
      // Replay the goal's own token: a ROS cancel carries no payload, and a
      // zero token is refused by the Arbiter under kEnforced.
      if (sink_)
        sink_->on_trajectory_cancel(
            {id, token}); // Supervisor -> kPreempted -> settle()
    } else {
      // Still planning, OR mid-handover. planner_.cancel() covers the first;
      // the second is covered by start_execution() re-reading cancel_requested
      // AFTER on_trajectory_accepted, because a cancel delivered before the
      // supervisor owns the goal drains against nothing on its single FIFO
      // inbox.
      planner_.cancel();
    }
  ...
  void handle_accepted(std::shared_ptr<GoalHandle> gh) {
    ...
    start_plan(
        *gh->get_goal(),
        ...
        [this, id](CuroboPlanClient::Outcome o) {
          on_plan_done(id, std::move(o));
        });
  }
  ...
  void on_plan_done(GoalId id, CuroboPlanClient::Outcome outcome) {
    ...
    if (gh->is_canceling()) { // canceled during planning; plan raced ahead and
                              // succeeded
    ...
    // Fail loud on malformed planner output rather than let
    // to_trajectory_goal's memory-safety zero-fill silently mis-map a
    // short/empty point onto the arm.
      ...
      if (p.positions.size() != static_cast<size_t>(kinova::kNumJoints)) {
        settle_local(gh, kPlanningFailed,
                     "planner returned malformed trajectory: point has " +
                         std::to_string(p.positions.size()) +
                         " positions, expected " +
    ...
    if (sink_->on_trajectory_goal(tg) !=
        kinova::interface::GoalResponse::kAccept) {
    ...
    // already finished (planner_.cancel() was a no-op) or the cancel reached
    // the supervisor's FIFO ahead of the goal and drained against no active
    ...
    kinova::interface::Token cancel_token{};
    {
      ...
      auto it = goals_.find(id);
      if (it != goals_.end()) {
        canceled = it->second.cancel_requested;
        cancel_token = it->second.token;
      }
    }
    if (canceled)
      sink_->on_trajectory_cancel({id, cancel_token});
  }

  void settle_local(std::shared_ptr<GoalHandle> gh, int error_code,
                    const std::string &msg) {
    terminal(gh, error_code, msg, kinova::JointVec::Zero());
  }

  // The single place a goal handle is terminated.
  void terminal(std::shared_ptr<GoalHandle> gh, int error_code,
                const std::string &msg, const kinova::JointVec &final_error) {
    ...
    try {
      ...
      RCLCPP_WARN(node_->get_logger(),
                  "goal terminal transition raced a cancel and was dropped: %s",
                  e.what());
    }
  }

  void erase(const GoalId &id) {
    std::lock_guard<std::mutex> l(m_);
    goals_.erase(id);
  }
  ...
  // token: the goal's arbitration capability, kept so cancel can replay it. A
  // ROS action cancel carries no payload, so a cancel with a zero token would
  // be REFUSED by the Arbiter under kEnforced and the motion would run on.
  struct Goal {
    std::shared_ptr<GoalHandle> gh;
    bool executing = false;

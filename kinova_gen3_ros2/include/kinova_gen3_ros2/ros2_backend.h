...
public:
  ...
  // Null until bringup wires it -- and null is a real configuration, not an
  // omission: a robot with no gripper publishes seven joints exactly as before
  // this tier.
  void set_gripper_sink(kinova::interface::GripperSink *sink) {
    gripper_ = sink;
  }
...
private:
  ...
  // surface saying so. Atomics because publish_state runs on the pump thread
  // and the updater on a ROS timer; there is no state here worth a mutex.
  ...
  std::mutex m_;
  // The goal's token, kept so cancel can replay it. A ROS action cancel carries
  // no payload (action_msgs/CancelGoal is one GoalInfo), so without this a
  // cancel would reach the Arbiter with a zero token and be REFUSED under
  // kEnforced -- the motion would keep running while the client believed it had
  // cancelled.
  struct Entry {

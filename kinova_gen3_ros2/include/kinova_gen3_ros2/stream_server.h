...
// /stream_status.
//
// Holds a StreamSink& and NOTHING else from core, so it unit-tests against a
// fake with no robot -- the same reason ArbitrationServer holds only an
// ArbitrationSink&.
//
...
public:
  ...
  // Publish only if core's stream state differs from what was last sent. Called
  // by the session handlers and by a 10 Hz timer, so a teardown core did on its
  // own (deadline expiry, halt) still surfaces.
  void publish_status_if_changed();

  // One controller: a control law plus the channels it reads. `core_backed` is
  // false when core has no SetpointKind for it at all, so there is nothing to
  // ask pair_supported() about.
  ...
  // Computed live from core, never declared: a controller is openable only if
  // core has a kind for it, that pair is supported, and it needs just one
  // channel (core admits exactly one SetpointKind per session).

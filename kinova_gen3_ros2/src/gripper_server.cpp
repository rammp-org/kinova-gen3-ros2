...
GripperServer::GripperServer(rclcpp::Node::SharedPtr node,
  ...
  // Best-effort, KeepLast(1): identical to the six arm setpoint topics, because
  // core's semantics are identical -- setpoints are absolute and latest-wins,
  // so dropping an intermediate one is correct rather than a loss.
  const auto sp_qos = rclcpp::QoS(rclcpp::KeepLast(1)).best_effort();
  // Its OWN mutually-exclusive group, isolated from the node's DEFAULT group --
  // which also carries /acquire_control, /release_control, /revoke_control and
  // the execute_joint_trajectory goal/cancel callbacks. on_setpoint blocks on
  // the Arbiter mutex, and StreamServer::on_open holds that mutex for a 250 ms
  // mode-settle sleep; sharing the default group would stall an operator's
  // /revoke_control or a trajectory cancel behind that sleep. Same reasoning as
  // StreamServer's setpoint_group_ and ArbitrationServer's estop_group_.
  ...
  sp_sub_ = node_->create_subscription<GripperSetpointMsg>(
      ...
      sp_opts);
  state_pub_ = node_->create_publisher<GripperStateMsg>(
...
void GripperServer::diagnostics(
...
void GripperServer::on_setpoint(const GripperSetpointMsg::SharedPtr m) {
  // Straight through to the sink, which is the ARBITER: that is what makes the
  // token load-bearing. A bad token is counted by the Arbiter, not refused
  // here.

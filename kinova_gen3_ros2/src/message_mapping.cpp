...
tol_to_vec(const std::vector<control_msgs::msg::JointTolerance> &t) {
  if (t.empty())
    return JointVec::Constant(-1.0); // disabled
  JointVec v = JointVec::Constant(-1.0);
  for (int i = 0; i < kNumJoints && i < static_cast<int>(t.size()); ++i)
    v[i] = t[i].position;
  return v;
}
...
// cuRobo plans jerky (see the driver's
// docs/deep-dive/trajectory-interpolation.md). A profile counts as present only
// when EVERY point carries a correctly sized one; a partial profile degrades to
// the next order down rather than mixing conventions mid-trajectory.
static void fill_trajectory(
  ...
  for (const auto &p : points) {
    ...
    for (int i = 0; i < kNumJoints && i < static_cast<int>(p.positions.size());
         ++i)
      w.q[i] = p.positions[i];
    if (all_vel)
      for (int i = 0; i < kNumJoints; ++i)
        w.qd[i] = p.velocities[i];
    if (all_acc)
      for (int i = 0; i < kNumJoints; ++i)
        w.qdd[i] = p.accelerations[i];
  ...
  tr.has_accelerations =
      all_vel && all_acc; // accelerations only count alongside velocities
}
...
TrajectoryGoal to_trajectory_goal(const ExecuteJointTrajectory::Goal &g) {
  ...
  tg.goal_time_tolerance_s =
      static_cast<double>(g.goal_time_tolerance.sec) +
      static_cast<double>(g.goal_time_tolerance.nanosec) * 1e-9;
  tg.control_mode = (g.control_mode == 1) ? ControlModeKind::kImpedance
                                          : ControlModeKind::kPosition;
  tg.preemption =
      (g.preemption == 1) ? Preemption::kLatestWins : Preemption::kQueue;
  tg.has_gains = (g.control_mode == 1);
  if (tg.has_gains) {
    for (int i = 0; i < kNumJoints; ++i) {
      tg.gains.kq[i] = g.gains.kq[i];
      tg.gains.torque_limit[i] = g.gains.torque_limit[i];
    }
...
ExecuteJointTrajectory::Feedback to_feedback_msg(const GoalId &,
...
to_gripper_setpoint(const kinova_gen3_interfaces::msg::GripperSetpoint &m) {
  ...
  // range is enforced for every caller, including unvalidated data off a
  // socket.
...
to_gripper_state_msg(const kinova::interface::GripperState &g) {
  ...
  // 20 Hz wall timer using node->now().

...
Ros2Backend::handle_goal(const rclcpp_action::GoalUUID &,
                         std::shared_ptr<const Action::Goal> goal) {
  if (!sink_)
    return rclcpp_action::GoalResponse::REJECT;
  // Fail loud, never silent mis-mapping (CLAUDE.md): the mapping layer
  // zero-fills a short point as a memory-safety net, but a malformed goal must
  // be REJECTED here.
  if (goal->trajectory.points.empty()) {
    RCLCPP_WARN(node_->get_logger(),
                "rejecting goal: trajectory has no points");
    return rclcpp_action::GoalResponse::REJECT;
  }
  for (size_t i = 0; i < goal->trajectory.points.size(); ++i) {
    ...
    if (n != static_cast<size_t>(kinova::kNumJoints)) {
      RCLCPP_WARN(node_->get_logger(),
                  "rejecting goal: point %zu has %zu positions, expected %d", i,
                  n, kinova::kNumJoints);
      return rclcpp_action::GoalResponse::REJECT;
    }
  }
...
Ros2Backend::handle_cancel(std::shared_ptr<GoalHandle> gh) {
  if (!sink_)
    return rclcpp_action::CancelResponse::REJECT;
  const GoalId id = gh->get_goal_id(); // GoalUUID -> GoalId
  // Replay the token the goal was ACCEPTED with. This is FUNCTIONAL, not a
  // security measure: a ROS cancel is unauthenticated by protocol
  // (action_msgs/CancelGoal has no payload, and a zero goal id cancels
  // everything) -- but without a valid token the Arbiter refuses the cancel
  // outright under kEnforced and the arm keeps moving.
...
void Ros2Backend::settle(const GoalId &id, const TrajectoryResult &r) {
  ...
  if (gh->is_canceling())
    gh->canceled(msg);
  else if (r.error_code == result_code::kSuccessful)
    gh->succeed(msg);
  else
    gh->abort(msg);
}
...
void Ros2Backend::publish_state(const ArmState &s) {
  ...
  if (gripper_ != nullptr) {
    // ONE joint: the 2F-85 is underactuated -- one revolute DOF and five
    // <mimic> joints at +/-1, which robot_state_publisher derives itself
    // (verified 2026-09-03). Publishing the dependents too would duplicate what
    // the model already states.
    //
    // Published REGARDLESS of `present`: omitting it would drop the gripper out
    // of TF entirely -- its links would just vanish from the model rather than
    // render at a frozen or default pose (verified 2026-09-03: RSP publishes
    // per-segment, not all-or-nothing for the whole robot; the arm's TF is
    // unaffected either way). /gripper_state carries the truth about presence
    // instead.
    //
    // This is a second snap_ load inside core, so the value can be up to one 1
    // kHz feedback frame newer than `s`. Irrelevant for TF; noted because the
    // conformance suite's same-pump-tick invariant covers joint_states and
    // ee_state, not this column.
    ...
    // indistinguishable from "not moving" / "no load". Core has no gripper
    // velocity at all (it removed the field), and its effort is a 0..1 current
    // fraction, not N*m.
...
void Ros2Backend::diagnostics(
  ...
  // STALE rather than OK when nothing has arrived: an arm we have never heard
  // from is not a healthy arm, and reporting OK for it is the failure mode REP
  // 107's levels exist to avoid.
  if (!ever)
    stat.summary(DiagnosticStatus::STALE, "no feedback from the arm yet");
  else if (fault)
    stat.summary(DiagnosticStatus::ERROR, "arm reports a fault");
  else

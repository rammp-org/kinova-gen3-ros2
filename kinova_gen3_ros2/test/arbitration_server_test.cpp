...
// failed ASSERT_* cannot destroy a joinable thread (std::terminate replaces the
// gtest diagnostic with a bare SIGABRT). Mirrors goto_ee_pose_integration_test.
class SpinThread {
public:
...
protected:
  ...
  void SetUp() override {
    ...
    plane_ = std::make_unique<kinova_gen3_ros2::ArbitrationServer>(node_, sink_,
                                                                   "test", 1.0);
  ...
  void TearDown() override {
    ex_.reset();
    plane_.reset(); // drops its publishers/subscriptions while the context is
                    // alive
    node_.reset();
    rclcpp::shutdown();
  }
  ...
  // Publishes on /estop and spins until ArbitrationServer has had a chance to
  // handle it.
  void publish_estop(bool engaged, const rclcpp::Time &stamp,
    ...
    m.source = src;
    SpinThread spin(*ex_);
    // Wait for the subscription to match before publishing, else the message
    // goes into a void and the test races.
  ...
  std::vector<rammp_arm_interfaces::msg::ControlStatus>
  collect_status(std::chrono::milliseconds dwell) {
    std::vector<rammp_arm_interfaces::msg::ControlStatus> got;
    std::mutex gm;
    auto sub =
        node_->create_subscription<rammp_arm_interfaces::msg::ControlStatus>(
...
// ArbitrationStatus deliberately does not carry the token, so ArbitrationServer
...
TEST_F(ArbitrationServerTest, ReleaseWithWrongTokenIsRefusedAndDoesNotRevoke) {
  ...
  EXPECT_EQ(sink_.log(),
            std::vector<std::string>{"grant:orchestrator"}); // no revoke
}
...
// A bag replay, or a clear delayed behind a network hiccup, must not re-enable
// a stopped arm. estop_clear_max_age_s is 1.0 in this fixture.
TEST_F(ArbitrationServerTest, StaleEstopClearIsIgnored) {
  publish_estop(true, node_->now(), "operator");
  publish_estop(false, node_->now() - rclcpp::Duration::from_seconds(30.0),
                "replay");
  ASSERT_FALSE(sink_.log().empty());
...
// a stale STOP is still honoured. Both branches must fail toward "arm stays
// stopped".
TEST_F(ArbitrationServerTest, StaleEstopEngageIsStillHonoured) {
  publish_estop(true, node_->now() - rclcpp::Duration::from_seconds(30.0),
                "replay");
  ASSERT_FALSE(sink_.log().empty());
  EXPECT_EQ(sink_.log().back(), "estop");
}

// `ros2 topic pub` leaves the stamp at zero. An e-stop control that cannot be
// driven from the CLI is worse than one that can be replayed.
TEST_F(ArbitrationServerTest, UnstampedEstopClearIsAccepted) {
  publish_estop(true, node_->now(), "operator");
  publish_estop(false, rclcpp::Time(0, 0, node_->get_clock()->get_clock_type()),
                "cli");
  ASSERT_FALSE(sink_.log().empty());
  EXPECT_EQ(sink_.log().back(), "estop_clear");
}
...
// REP 107: reporting is on /diagnostics using diagnostic_msgs/DiagnosticArray
// at 1 Hz. Level must be ERROR while e-stopped -- this is what a monitoring
// dashboard reads.

...
struct FakeSupervisor : public CommandSink {
  ...
  void on_trajectory_accepted(const GoalId &id,
                              const TrajectoryGoal &) override {
    TrajectoryResult r;
  ...
  void on_halt(HaltReason why) override {
    halted = true;
    halt_reason = why;
  }
...
// lives under: a cancel only stops motion if it reaches the supervisor AFTER
// the goal has been handed over. The real Supervisor queues both on one FIFO
// inbox, so a cancel delivered first drains against no active goal and is
// silently lost -- and the trajectory then runs to completion.
//
// It blocks inside on_trajectory_goal until the test releases it, which pins
// the cancel to exactly the window between the supervisor accepting the goal
// and the server handing it over. Ordering is recorded, not timed.
...
// supervisor must still stop the arm. Before the fix the cancel was ACCEPTed
// and then dropped: it either hit planner_.cancel() on an already-finished
// plan, or reached the supervisor AHEAD of the goal, where it drained against
// nothing. Either way the client saw a cancel it never got, and the arm
// executed the entire cuRobo motion at planned speed.
TEST_F(GotoServerTest, CancelDuringHandoverStillReachesTheSupervisor) {
  ...
  ASSERT_GE(seen.size(), 3u)
      << "supervisor never saw the cancel; the arm would "
...
// The planner must not have to discover where the arm is: kinova_gen3_node owns
...
// read, and a default-constructed ArmState is {q = Zero, stamp_s = 0}. Before
// the first good frame, sending that q as the start state is indistinguishable
// from a real measurement, and cuRobo would plan from the fully-extended zero
// pose. The goal must be refused instead -- the loud failure this replaced
// lived on the planner side ("no fresh joint state on /joint_states").
TEST_F(GotoServerTest, GoalIsRefusedWhenNoJointStateHasBeenMeasuredYet) {
  ...
  EXPECT_FALSE(fut.get())
      << "goal must be refused when the arm state is unknown";
...
// SUCCESS but its first point carries only 4 positions instead of 7.
// on_plan_done must reject this before mapping/submitting rather than let
// to_trajectory_goal's zero-fill silently mis-map a short point onto the arm.

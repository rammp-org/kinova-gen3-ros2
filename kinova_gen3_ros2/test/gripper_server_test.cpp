...
TEST_F(GripperServerTest, ASetpointReachesTheSinkWithAllThreeFields) {
  ...
  m.force = 0.2f;
  m.token[0] = 3;
  for (int i = 0; i < 20 && sink.count() == 0; ++i) {
    pub->publish(m);
    spin_for(20ms);
  }
...
TEST_F(GripperServerTest, PublishStateReportsWhatTheSinkSays) {
  FakeGripperSink sink;
  sink.state.position = 0.3f;
  sink.state.effort = 0.05f;
  sink.state.current = 0.05f;
...
namespace {
// diagnostic_updater::Updater's task/level are not reachable from outside
// GripperServer (updater_ is private, as it should be), so the only testable
// seam is the same one the conformance suite uses: subscribe to /diagnostics
// and let the updater's own periodic timer (1 Hz) publish it. Slower than the
// other tests here, but real -- it exercises GripperServer::diagnostics()
// exactly as it runs in the node.
diagnostic_msgs::msg::DiagnosticStatus
wait_for_gripper_task(rclcpp::Node::SharedPtr node,
                      std::function<void(std::chrono::milliseconds)> spin_for) {
  ...
  auto sub = node->create_subscription<diagnostic_msgs::msg::DiagnosticArray>(
      ...
      [&](diagnostic_msgs::msg::DiagnosticArray::SharedPtr m) {
        for (const auto &s : m->status) {
          if (s.name.find("Gripper") != std::string::npos) {
            got = s;
            seen = true;
          }
        }
  ...
  return seen
             ? got
             : throw std::runtime_error("no Gripper task seen on /diagnostics");
}
} // namespace
...
// gripper out of TF entirely -- its links would just vanish from the model
// rather than render at a frozen or default pose (verified 2026-09-03:
// robot_state_publisher publishes per-segment, not all-or-nothing for the whole
// robot; the arm's own TF is unaffected either way). /gripper_state.present is
// where absence is reported truthfully.
TEST_F(GripperServerTest, KnuckleJointIsPublishedEvenWhenAbsent) {
  FakeGripperSink sink;
  sink.state.position = 0.5f;
  sink.state.present = false;

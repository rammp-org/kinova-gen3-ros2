...
inline constexpr int kGoalRejected =
    904; // the server refused the goal outright
...
struct FakeSupervisor : public kinova::interface::CommandSink {
  ...
  kinova::interface::GoalResponse
  on_trajectory_goal(const kinova::interface::TrajectoryGoal &g) override {
    last_goal = g;
    got_goal = true;
  ...
  void
  on_trajectory_accepted(const kinova::interface::GoalId &id,
  ...
  kinova::interface::CancelResponse
  on_trajectory_cancel(const kinova::interface::CancelRequest &req) override {
    kinova::interface::TrajectoryResult r;
...
struct DummyPort : public kinova::interface::ActionServerPort {
  void
  publish_feedback(const kinova::interface::GoalId &,

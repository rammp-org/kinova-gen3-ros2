...
class C(Node):
    def __init__(self, args):
        ...
        self.cli = ActionClient(
            self, ExecuteJointTrajectory, "execute_joint_trajectory"
        )
    ...
    def run(self):
        ...
        p1.positions = goal
        s = int(self.args.dur)
        p1.time_from_start = Duration(sec=s, nanosec=int((self.args.dur - s) * 1e9))
        g.trajectory.points = [p0, p1]
        if self.args.path_tol > 0:
            jt = JointTolerance()
            jt.position = self.args.path_tol
        ...
        self.code = rf.result().result.error_code
        self.status = rf.result().status
        print(f"result error_code={self.code} status={self.status}")

    def on_fb(self, fb):
        meas = "  ".join(
            f"j{j}={fb.feedback.actual.positions[j]:+.4f}" for j, _ in self.args.moves
        )
...
def main():
    ...
    if len(joints) != len(deltas):
        print(
            f"ERROR: --joint ({len(joints)}) and --delta ({len(deltas)}) count mismatch"
        )
        sys.exit(2)
    if any(not (0 <= j <= 6) for j in joints):
        print(f"ERROR: every --joint must be in [0,6], got {joints}")
        sys.exit(2)
